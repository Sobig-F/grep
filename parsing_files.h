...
regmatch_t **append_pm(regmatch_t **all_pm, regmatch_t *pm, int *len,
                       int offset) {
  ++(*len);
  if (all_pm == NULL) {
...
regmatch_t **check_templates(regex_t *template, char *str, int *len,
                             regmatch_t **all_pm) {
  int offset = 0;
  regmatch_t *pm = NULL;
...
int check_string(char *filename, char *str, regex_t **templates,
                 struct flags *flags, int count_str, int *print_str,
                 int count_files) {
  int find_count = 0;
  ...
  if (all_pm != NULL) {
    all_pm =
        (regmatch_t **)realloc(all_pm, sizeof(regmatch_t *) * (find_count + 1));
    all_pm[find_count] = NULL;
  }
  ...
  for (int i = 0; (i < find_count) && (*print_str || !flags->o); ++i) {
    if (!flags->h && !flags->c && !flags->l && count_files > 1) {
      printf("%s:", filename);
    }
    if (flags->n) {
      printf("%d:", count_str);
...
void parsing_file(char *filename, FILE *file, regex_t **templates,
                  struct flags *flags, int *print_str, int count_files) {
  char *str = NULL;
...
void parsing_all_filepath(struct findStruct *grepStruct) {
  FILE *file = NULL;
  if ((grepStruct->files != NULL) && (grepStruct->templates != NULL) &&
      (grepStruct->flags != NULL)) {
    for (int i = 0; *(grepStruct->files + i) != NULL; ++i) {
      file = fopen(*(grepStruct->files + i), "r");
      if (file != NULL) {
        parsing_file(*(grepStruct->files + i), file, grepStruct->templates,
                     grepStruct->flags, &grepStruct->print_str,
                     grepStruct->count_files);
      }
      fclose(file);
    }
  }
