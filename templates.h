...
char **association_templates(FILE *file, char **templates, int *count,
  ...
  while (str != NULL) {
    ...
    *count += 1;
    ...
    templates[*count - 1] =
        (char *)malloc(sizeof(char) * ((int)strlen(str) + 1));
...
char **append_argument(char **array, int *count, char **argv, int indexj,
                       int indexi, int *error) {
  if (array == NULL) {
  ...
  if (indexj != (int)strlen(argv[indexi]) - 1) {
    ...
    array[*count - 1] =
        (char *)malloc(sizeof(char) * (strlen(argv[indexi]) - indexj - 1));
    ...
    if ((argv[indexi + 1] != NULL) && ((int)strlen(argv[indexi + 1]) > 1)) {
      ...
      array[*count - 1] = NULL;
      array[*count - 1] =
          (char *)malloc(sizeof(char) * ((int)strlen(argv[indexi + 1]) + 1));
...
char **find_feflags_argument(char **argv, char **array, char flag, int *count) {
  int found, error = 0;
  for (int i = 1; (*(argv + i) != NULL) && !error; ++i) {
    found = 0;
    for (int j = 0; (j < (int)strlen(argv[i])) && (argv[i][0] == '-') && !found;
         ++j) {
      if (((flag == 'e') && (argv[i][j] == 'f')) ||
          ((flag == 'f') && (argv[i][j] == 'e'))) {
        found = 1;
      }
      if ((argv[i][j] == flag) && !found) {
        found = 1;
        array = append_argument(array, count, argv, j, i, &error);
      }
    }
  }
  if (array != NULL) {
...
char **find_first_temp(char **argv, char **templates, int *count) {
  for (int i = 1; templates == NULL; ++i) {
    if (argv[i][0] != '-') {
      ...
      if (templates != NULL) {
        templates[0] = NULL;
        *templates = (char *)malloc(sizeof(char) * (strlen(argv[i]) + 1));
...
regex_t **find_template(char **argv, struct flags *flags, int *print_str) {
  ...
  if (flags->f) {
    paths_regulars =
        find_feflags_argument(argv, paths_regulars, 'f', &count_path);
    if (paths_regulars != NULL) {
      templates =
          run_regulars_file(paths_regulars, templates, &count, print_str);
